...
const webworkify = require('webworkify'),
    webWorker = require('./process-web-worker'),
...
const rsaFunc = function() {
    ...
    childProcess = webworkify(webWorker);

    childProcess.addEventListener('message', processData);

    promise.itsa_finally(() => {
        // make sure the worker is closed:
        childProcess.removeEventListener('message', processData);
    });

    promise.kill = promise.reject.bind(promise);

    // sending arguments to the child-process:
    childProcess.postMessage(args);
